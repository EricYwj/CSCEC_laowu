...
public class WorkVisacancelControllerBean extends AbstractWorkVisacancelControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.zjlw.certificates.app.WorkVisacancelControllerBean");
 // 保存
	protected IObjectPK _save(Context ctx, IObjectValue model)
			throws BOSException, EASBizException {
		WorkVisacancelInfo accInfo = (WorkVisacancelInfo) model;
...
//		}
	...
	protected void _passAudit(Context ctx, IObjectPK pk, IObjectValue model)
		...
		WorkVisacancelInfo info = (WorkVisacancelInfo) getValue(ctx, pk, getSelector());
	 ...
	 */
	private void updatePerHis(Context ctx, WorkVisacancelInfo info,boolean audMark)
			throws BOSException, EASBizException {
		WorkVisacancelEntryCollection entryCol = info.getEntrys();
		for (int i = 0; i < entryCol.size(); i++) {
			...
			for (int j = 0; j < phCol.size(); j++) {
				...
				hisInfo.setBsnisState(audMark?perBizStateEnum.VISA:perBizStateEnum.ANTISIGNED);
		...
		protected void _unpassAudit(Context ctx, IObjectPK pk, IObjectValue model)
				throws EASBizException, BOSException {
			WorkVisacancelInfo info = (WorkVisacancelInfo) getValue(ctx, pk,
					getSelector());
			FilterInfo filter = new FilterInfo();
