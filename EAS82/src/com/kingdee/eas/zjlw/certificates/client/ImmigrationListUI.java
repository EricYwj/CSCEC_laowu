 ...
 * output class name
 */
public class ImmigrationListUI extends AbstractImmigrationListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ImmigrationListUI.class);
  ...
  //审核
    public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
    	...
    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			...
			ImmigrationFactory.getRemoteInstance().passAudit(new ObjectUuidPK(id), null);
		}
    	showOKMsgAndRefresh();
    ...
    public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
    	...
    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			String id = (String) iter.next();
			ImmigrationFactory.getRemoteInstance().unpassAudit(new ObjectUuidPK(id), null);
		}
...
//				//签证到期日期；入境前，预警黄色：30天<=签证到期日期-today<=45天；
//				if(row.getCell("entrys.veTime").getValue()!=null&&row.getCell("entrys.personID").getValue()!=null){
...
//							if(dayCou>=30&&dayCou<=45){
//								row.getCell("entrys.veTime").getStyleAttributes().setBackground(Color.yellow);
//							}
//						}
//					}
//				}
...
//	 */
//	public void setTimeAndCheckCancel(ImmigrationInfo info) throws BOSException, EASBizException {
//		String bNum = info.getContbNum();
//		Date dqFTime = null;
//		FilterInfo filter = new FilterInfo();
//		filter.getFilterItems().add(new FilterItemInfo("bNum",bNum));
//		SelectorItemCollection sic = new SelectorItemCollection();
//		sic.add("entrys.qualDate");
//		EntityViewInfo ev = new EntityViewInfo();
//		ev.setFilter(filter);
//		ev.setSelector(sic);
//		DoubQualifyCollection dqCol = DoubQualifyFactory.getRemoteInstance().getDoubQualifyCollection(ev);
//		DoubQualifyInfo dbinfo = dqCol.get(0);
//    	DoubQualifyEntryCollection dqEntryCol = dbinfo.getEntrys();
//    	DoubQualifyEntryInfo dqEntryInfo = new DoubQualifyEntryInfo();
//		dqEntryInfo = dqEntryCol.get(0);
...
//	}
    ...
    /*added by wangth om 20170711 start */
...
//		view.setSorter(sic);
	...
	protected void prepareUIContext(UIContext uiContext, ActionEvent e) {
       ...
       /*added by wangth om 20170711 end */
	
    /**
     * output class constructor
     */
    public ImmigrationListUI() throws Exception
    {
        super();
    }
    ...
    public void actionEdit_actionPerformed(ActionEvent e) throws Exception
    {
		...
		if (pkList.size() > 1) {
	    	...
	    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
				String id = (String) iter.next();
				ImmigrationInfo info = ImmigrationFactory.getRemoteInstance().getImmigrationInfo(new ObjectUuidPK(id));
				ImmigrationEntryCollection antiEtyCol = info.getEntrys();
				for (int i = 0; i < antiEtyCol.size(); i++) {
					ImmigrationEntryInfo etyInfo = antiEtyCol.get(i);
					etys.add(etyInfo);
				}
			}
	    	//进行参数传递
	        ...
	        uiContext.put("etys", etys);
	       	IUIWindow ui = UIFactory.createUIFactory(UIFactoryName.NEWTAB).create("com.kingdee.eas.zjlw.certificates.client.ImmiEcEditUI", uiContext, null, OprtState.ADDNEW);
	       	ui.show();
		}else{
...
//    	updateDqTime(pkList);//更新双认证完成时间
     ...
     * @throws BOSException
     */
//    private void updateDqTime(List pkList) throws EASBizException, BOSException {
//    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
//			String id = (String) iter.next();
//			SelectorItemCollection sic = new SelectorItemCollection();
//			sic.add("number");
//			sic.add("entrys.IdNum");
//			sic.add("entrys.passpNum");
//			ImmigrationInfo info = ImmigrationFactory.getRemoteInstance().getImmigrationInfo(new ObjectUuidPK(id), sic);
//			String idNum = info.getEntrys().get(0).getIdNum();
//			String passpNum = info.getEntrys().get(0).getPasspNum();
//			FilterInfo filter = new FilterInfo();
...
//			info.getEntrys().get(0).setDbCmpTime(d);
//			ImmigrationFactory.getRemoteInstance().update(new ObjectUuidPK(info.getId()), info);
//		}		
//	}
	/**
