 ...
 * output class name
 */
public class EmbassyRegListUI extends AbstractEmbassyRegListUI
{
    private static final Logger logger = CoreUIObject.getLogger(EmbassyRegListUI.class);
...
//	    	filter.getFilterItems().add(new FilterItemInfo("creator.id", userInfo.getId()));
    ...
    public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
    	...
    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			...
			EmbassyRegFactory.getRemoteInstance().passAudit(new ObjectUuidPK(id), null);
		}
    	showOKMsgAndRefresh();
    ...
    public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
    	...
    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			String id = (String) iter.next();
			EmbassyRegFactory.getRemoteInstance().unpassAudit(new ObjectUuidPK(id), null);
		}
    	showOKMsgAndRefresh();
    	EcClientHelper.showOprtOK(this);
    	checkDqDate();
     ...
     */
    public EmbassyRegListUI() throws Exception
    {
        super();
    }
...
//    		IRow row=tblMain.getRow(i);
    		//递送使馆注册资料时间颜色预警；条件：签证到期日不为空且不是补办
//    		if(row.getCell("entrys.veTime").getValue()!=null){
//    			Date veTime=(Date) row.getCell("entrys.veTime").getValue();
//    			//值为零时：1.黄色预警：7天<=签证到期日-today<=15天；2.红色预警：签证到期日-today<7天。
//    			if(row.getCell("entrys.sbmEmbTime").getValue()==null){
//    				long time=(veTime.getTime()-date.getTime())/86400000;
//    				int dayCou = Integer.parseInt(String.valueOf(time)); 
//    				if(dayCou>=7 && dayCou<=15){
//    					row.getCell("entrys.sbmEmbTime").getStyleAttributes().setBackground(Color.yellow);
//    				}else if(dayCou<7){
//    					row.getCell("entrys.sbmEmbTime").getStyleAttributes().setBackground(Color.red);
//    				}
//    			}else{
//    				//值不为零时：红色预警：签证到期日-递交资料日期<0
//    				Date sbmEmbTime=(Date) row.getCell("entrys.sbmEmbTime").getValue();
//    				long time=(veTime.getTime()-sbmEmbTime.getTime())/86400000;
//    				int dayCou = Integer.parseInt(String.valueOf(time)); 
//    				if(dayCou<0){
//    					row.getCell("entrys.sbmEmbTime").getStyleAttributes().setBackground(Color.red);
//    				}
...
//		view.setSorter(sic);
    ...
    public void actionEdit_actionPerformed(ActionEvent e) throws Exception
    {
		...
		if (pkList.size() > 1) {
	    	...
	    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
				String id = (String) iter.next();
				EmbassyRegInfo info = EmbassyRegFactory.getRemoteInstance().getEmbassyRegInfo(new ObjectUuidPK(id));
				...
				EmbassyRegEntryCollection antiEtyCol = info.getEntrys();
				for (int i = 0; i < antiEtyCol.size(); i++) {
					EmbassyRegEntryInfo etyInfo = antiEtyCol.get(i);
					etys.add(etyInfo);
				}
			}
	    	//进行参数传递
	        ...
	        uiContext.put("etys", etys);
	       	IUIWindow ui = UIFactory.createUIFactory(UIFactoryName.NEWTAB).create("com.kingdee.eas.zjlw.certificates.client.EpEcEditUI", uiContext, null, OprtState.ADDNEW);
	       	ui.show();
		}else{
