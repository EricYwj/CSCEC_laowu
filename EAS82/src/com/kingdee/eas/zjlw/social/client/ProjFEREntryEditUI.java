    ...
    protected void afterEditStopped(KDTable table, Object oldValue,Object newValue, int colIndex, int rowIndex) throws Exception {
		...
		if("personSoclNum".equals(key)){
			...
			if(personSoclNum!=null){
				...
				if(fpInfo!=null){
					row.getCell("lastName").setValue(fpInfo.getFirstNameApl());//姓getFirstNameApl
					row.getCell("firstName").setValue(fpInfo.getLastNameApl());//名getLastNameApl
...
//    		if(personSoclNum==null){
//    			MsgBox.showInfo("此单据项目个人社保号为空，无法提交，请填入!");
//        		this.abort();
//    		}
    		if(personCredits==null){
    			MsgBox.showInfo("此单据项目个人返还金额为空，无法提交，请填入!");
        		this.abort();
    		}
    	}
    	if(projName==null){
    		MsgBox.showInfo("此单据项目名称为空，无法提交，请填入项目名称");
    		this.abort();
    	}else{
    		if(years==null){
    			MsgBox.showInfo("此单据年月为空，无法提交，请填入项目名称");
        		this.abort();
    		}
    	}
    	FilterInfo filter = new FilterInfo();
		...
		if (checkCol.size() > 1 ) {
			MsgBox.showInfo("该项目所选年月已有项目已存在休假工资录入表，不允许重复添加！");
