...
import com.kingdee.eas.zjlw.social.ProjFERAssdEntryFactory;
...
import com.kingdee.eas.zjlw.social.ProjFEREntryEntryFactory;
 ...
 * output class name
 */
public class ProjFERAssdEditUI extends AbstractProjFERAssdEditUI {
	private static final Logger logger = CoreUIObject.getLogger(ProjFERAssdEditUI.class);

	/**
	 * output class constructor
	 */
	public ProjFERAssdEditUI() throws Exception {
		super();
	}
	...
	public void onLoad() throws Exception {
		...
		this.kdtEntrys.getColumn("fWorkCount").getStyleAttributes().setNumberFormat("###,###,##0.00");
	}

	protected void setButtonStatus() {
		ProjFERAssdInfo bill;
		if ("VIEW".equals(getOprtState())) {
			// this.actionAudit.setVisible(true);
			// this.actionUnAudit.setVisible(true);
			// this.actionAudit.setEnabled(true);
			// this.actionUnAudit.setEnabled(true);
			bill = (ProjFERAssdInfo) this.editData;
	...
	private void initAll() throws BOSException, EASBizException {
		ProjFEREntryInfo pwInfo = getAlgPayrollData();// 查询恶劣天气返还录入
		if (pwInfo == null) {
	...
	private ProjFEREntryInfo getAlgPayrollData() throws BOSException, EASBizException {
		FilterInfo filter = new FilterInfo();
		AdminOrgUnitInfo projInfo = (AdminOrgUnitInfo) this.prmtprojName.getValue();// prmtprojName
		ProjFERAssdInfo info = ProjFERAssdFactory.getRemoteInstance().getProjFERAssdInfo(new ObjectUuidPK(this.editData.getId()));
		...
		ProjFEREntryInfo pwInfo = ProjFEREntryFactory.getRemoteInstance().getProjFEREntryCollection(view).get(0);
		return pwInfo;
	}

	private void inniEntry(ProjFEREntryInfo pwInfo) throws EASBizException, BOSException {
		// 获取工资数据的泛型集合
			...
			for (Object coopObj : coopSet) {
				...
				if (coopInfo != null) {
					...
					for (Object obj : pwEntrySet) {
						ProjFEREntryEntryInfo pwEntryInfo = (ProjFEREntryEntryInfo) obj;

						AdminOrgUnitInfo cooperCode = pwEntryInfo.getCooperCode();
						if (cooperCode != null && cooperCode.getId() != null) {
					...
					row.getCell("forgWorkerNum").setValue(coopAlgSc);
					row.getCell("fWorkCount").setValue(wthReturnCount);
				}
			}
		}
	}

	private void inniHead(ProjFEREntryInfo pwInfo) throws EASBizException, BOSException {
		int algScCount = 0;
		BigDecimal wthReturnCount = BigDecimal.ZERO;
		if (pwInfo != null) {
			ProjFEREntryEntryCollection pwEntryCol = pwInfo.getEntrys();
			for (int i = 0; i < pwEntryCol.size(); i++) {
				ProjFEREntryEntryInfo pwEntryInfo = pwEntryCol.get(i);
		...
		row.getCell("forgWorkerNum").setValue(algScCount);
		row.getCell("fWorkCount").setValue(wthReturnCount);
	}

	private Set getPayObjectSet(ProjFEREntryInfo pwInfo) {
		Set<Object> set = new HashSet<Object>();
		ProjFEREntryEntryCollection pwEntryCol = new ProjFEREntryEntryCollection();
		if (pwInfo != null) {
	...
	private Set getCoopSet(Set paySet) throws EASBizException, BOSException {
		...
		for (Object obj : paySet) {
			ProjFEREntryEntryInfo pay = (ProjFEREntryEntryInfo) obj;
			if (pay != null && pay.getCooperCode() != null && pay.getCooperCode().getId() != null) {
	...
	protected void afterEditStopped(KDTable table, Object oldValue, Object newValue, int colIndex, int rowIndex) throws Exception {
		...
		// view.setFilter(filter);
		// ProjFEREntryEntryCollection col=ProjFEREntryEntryFactory.getRemoteInstance().getProjFEREntryEntryCollection(view);
		// if(col!=null&&col.size()>0){
		// for(int i=0;i<col.size();){
		// ProjFEREntryEntryInfo pjInfo=col.get(i);
		// if(pjInfo!=null){
		// count=(pjInfo.getPersonCredits()).add(count);
		// }
		// num=++i;
		// }
		// row.getCell("forgWorkerNum").setValue(num);
		// row.getCell("fWorkCount").setValue(count);
		// }else{
		// row.getCell("forgWorkerNum").setValue(null);
		// row.getCell("fWorkCount").setValue(null);
		// }
		// }
		// }
	}
