	...
	public void onLoad() throws Exception {
		...
		if (getOprtState().equals("ADDNEW")) {
			...
			for (Object object : etys) {
				...
				row.getCell("firstName").setValue(etyInfo.getFirstName());// 姓
				row.getCell("lastName").setValue(etyInfo.getLastName());// 名
				...
				row.getCell("birthPlaceFr").setValue(etyInfo.getBirthPlaceFr());// 出生地法文
				...
				if (etyInfo.getPassportCityCC() != null) {
					...
					row.getCell("passportCityCC").setValue(passportCityCC);// 护照签发地
																			// （
																			// 中文
																			// ）
				}
		...
		for (int i = 0; i < col.size(); i++) {
			...
			if (boaCol != null && boaCol.size() > 0) {
				for (int j = 0; j < boaCol.size(); j++) {
					...
					newBoaInfo.setAssoBusObjType("19A0BB9D");// 关联业务对象的类型：
																// 原劳动证分录BOSTYPE
