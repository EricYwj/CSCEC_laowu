...
import com.kingdee.eas.zjlw.social.VabaweatherFactory;
import com.kingdee.eas.zjlw.social.app.PayrollControllerBean;

/**
 * output class name
 */
public class PayrollListUI extends AbstractPayrollListUI {
	private static final Logger logger = CoreUIObject.getLogger(PayrollListUI.class);
	...
	public void onLoad() throws Exception {
		...
		this.btnUpdateErrData.setEnabled(true);
		...
		if ("L2".equals(strLanguage) || "l2".equals(strLanguage)) {
			...
			setUITitle("Journaux de paie  (Algérien)");
		}
	...
	public void actionUpdateErrData_actionPerformed(ActionEvent e) throws Exception {
		super.actionUpdateErrData_actionPerformed(e);
		FullOrgUnitInfo orgUnitInfo = getSelectedProjectOrgInfo();
		if (orgUnitInfo != null && orgUnitInfo.getId() != null)
			PayrollFactory.getRemoteInstance().updateErrData(orgUnitInfo.getId().toString());
	}
	...
	protected FilterInfo getDefaultFilterForQuery() {
		...
		if (userOrgIdList.size() != 0) {
			filter.getFilterItems().add(new FilterItemInfo("projName.id", userOrgIdSet, CompareType.INCLUDE));
	...
	public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			...
			PayrollFactory.getRemoteInstance().passAudit(new ObjectUuidPK(id), null);
		}
		showOKMsgAndRefresh();
	...
	public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		if (MsgBox.showConfirm2(this, "您确认要反审核吗？") != 0)
			return;
		List pkList = EcClientHelper.getSelectedIdValues(tblMain, getKeyFieldName());
		// ******************************校验是否有未删除的项目社保和休假工资分摊表 ywj 2018-2-24***************************************
		for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			String id = (String) iter.next();
			PayrollInfo info = PayrollFactory.getRemoteInstance().getPayrollInfo(new ObjectUuidPK(id), getSelectorsForUnAudit());
			FilterInfo filter = new FilterInfo();
			filter.getFilterItems().add(new FilterItemInfo("projName.id", info.getProjName().getId().toString()));
			filter.getFilterItems().add(new FilterItemInfo("monthYearr.id", info.getMonthYearr().getId().toString()));
			EntityViewInfo view = new EntityViewInfo();
			view.setFilter(filter);
			SecuSplitCollection SecuSplitCol = SecuSplitFactory.getRemoteInstance().getSecuSplitCollection(view);
			if (SecuSplitCol != null && SecuSplitCol.size() != 0) {
				MsgBox.showInfo("反审核前需删除关联的【项目社保和休假工资分摊表】！  " + info.getProjName().getName() + "存在未删除的项目社保和休假工资分摊表。");
				abort();
			}
		}
		// ******************************校验是否有未删除的项目社保和休假工资分摊表 ywj 2018-2-24***************************************
		for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			String id = (String) iter.next();
			PayrollFactory.getRemoteInstance().unpassAudit(new ObjectUuidPK(id), null);
		}
	 ...
	 * 返回sic
	 * 
	 * @author ywj 2018-2-24
	 * @return
	 */
	private SelectorItemCollection getSelectorsForUnAudit() {
		SelectorItemCollection sic = new SelectorItemCollection();
		sic.add("id");
		sic.add("name");
		sic.add("number");
		sic.add("projName.id");
		sic.add("projName.name");
		sic.add("monthYearr.id");
		return sic;
	}
	...
	protected FilterInfo getOrgTreeQuery() {
		...
		FullOrgUnitInfo orgUnitInfo = getSelectedProjectOrgInfo();
		if (orgUnitInfo != null) {
			String orgFrsNum = orgUnitInfo.getNumber().substring(0, 1);
			String filterMark = "";
			FilterInfo projectOrgFilter = new FilterInfo();
			if ("F".equals(orgFrsNum)) {
				filterMark = "projName.longNumber";
			} else {
			...
			// new FilterItemInfo("permitOrg.longNumber", orgUnitInfo
			// .getLongNumber()
			// + "!%", CompareType.LIKE));
			//
			// projectOrgFilter.getFilterItems().add(
	 ...
	 */
	public PayrollListUI() throws Exception {
		super();
	}
