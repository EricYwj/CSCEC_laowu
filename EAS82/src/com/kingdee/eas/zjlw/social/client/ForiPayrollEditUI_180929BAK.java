 ...
 * output class name
 */
public class ForiPayrollEditUI_180929BAK extends AbstractForiPayrollEditUI {
	private static final Logger logger = CoreUIObject.getLogger(ForiPayrollEditUI_180929BAK.class);

	/**
	 * output class constructor
	 */
	public ForiPayrollEditUI_180929BAK() throws Exception {
		super();
	}
	...
	public void onLoad() throws Exception {
		...
		// ***************************2018-6-4 ywj
		// ********************************begin
		// 2018-6-4 ywj IRG征收基数错误 因客户不会自行修改征收基数等字段，所以直接赋值
		// if(getOprtState().equals("EDIT")||getOprtState().equals("VIEW")){
		// if(this.txtacountRow.getText()==null||"".equals(this.txtacountRow.
		// getText())){
		// checkSociaLevyBase();
		// this.txtacountRow.setText("1");
		// }
		// }
	...
	private void checkCount() {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			/**
			 * 基本工资 basePay 出勤天数 nWorkDay 月出勤工资 attMthWage 平时加班 normalOver 节假日加班 festOver 平时加班小时工资基数 nmlBsWgPerHour 节假日加班小时工资基数 hldBsWgPerHour 平时加班工资总额 totNmlBsWg 节假日加班工资总额 totHldBsWg 1.月出勤工资 等于=基本工资/22*出勤天数 2.平时加班小时工资基数 等于=基本工资/173*1.5 3.节假日加班小时工资基数 等于=基本工资/173*2 4.平时加班工资总额 等于=平时加班小时工资基数*平时加班/小时 5.节假日加班工资总额 等于=节假日加班小时工资基数*节假日加班/小时
			 * 
			 * 
			 * 
			 * 
			 * 
			 * 社保征收基数 SociaLevyBase = 1. attMthWage 月出勤工资+ 2. seniWage 工龄工资（工龄补贴）+ 3. totNmlBsWg 平时加班工资总额+ 4. totHldBsWg 节假日加班工资总额+ 7. urgeWage 月鼓励补贴总额（鼓励补贴）+ 8. oneTimeWage 个人绩效奖金总额+ 10. riskWage 现场风险补贴总额+ 11. disasWage 危害补贴总额+ 12. langWage 语言补贴
			 * 
			 ...
			 * 
			 * 
			 * 净工资 等于= grossPay 毛工资- totCharge 扣款总额
			 */
			String workDate = "22";
			...
			// ）
			/*
			 * ywj 2018-4-24 因去除外会计算字段，所以校验去除 //社保征收基数（外会计算） soLevyBaseW 社保征收基数 SociaLevyBase 如果和系统计算不一致则标红 if(soLevyBaseW!=null&&SociaLevyBase!=null){ if(!(soLevyBaseW.compareTo(SociaLevyBase)==0)){ row.getCell("soLevyBaseW" ).getStyleAttributes().setBackground(Color.red);// } } //IRG征收基数（外会计算） IRGLBaseW IRG征收基数 IRGLBase 如果和系统计算不一致则标红 if(IRGLBaseW!=null&&IRGLBase!=null){ if(!(IRGLBaseW.compareTo(IRGLBase)==0)){ row.getCell("IRGLBaseW"). getStyleAttributes().setBackground(Color.red);// } } //IRG扣款（外会计算） IRGDeduction IRG扣款（个人所得税） IRGDPerson 如果和系统计算不一致则标红 if(IRGDeduction!=null&&IRGDPerson!=null){ if(!(IRGDeduction.compareTo(IRGDPerson)==0)){ row.getCell("IRGDeduction" ).getStyleAttributes().setBackground(Color.red);// } }
			 */
			if (nWorkDay != null) {
				// BigDecimal monthWork=new BigDecimal(nWorkDay);
				if (basePay != null) {
					if (wkDate != null) {
	...
	public void checkSociaLevyBase() {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			// if (b1.compareTo(e1) <= 0) {
			// b1 = e1;
			// row.getCell("IRGDPerson").setValue((a1.subtract(b1)).multiply(coefficient));
			// row.getCell("IRGDPerson").getValue();
			// }
			// if (b1.compareTo(e1) > 0 && b1.compareTo(e2) <= 0) {
			// b1 = b1;
			// row.getCell("IRGDPerson").setValue((a1.subtract(b1)).multiply(coefficient));
	...
	public void checkPrice() {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			// )
			// 总额
			// 基本工资/22*0.75*（22-出勤天数） zxh 2016-10-11
			// 等于=基本工资/22*0.75*Intempérie(53 km)/J zxh 2016-10-13 【公式更新】
	...
	protected void afterEditStopped(KDTable table, Object oldValue, Object newValue, int colIndex, int rowIndex) throws Exception {
		...
		BigDecimal a = new BigDecimal(0.09);
		BigDecimal b = new BigDecimal(0.00375);
		// 社保征收基数（外会计算） 社保征收基数 SociaLevyBase 如果和系统计算不一致则标红
		// if("soLevyBaseW".equals(key)){ ywj 2018-4-24
		// //BigDecimal soLevyBaseW=(BigDecimal)
		// row.getCell("soLevyBaseW").getValue();
		// //BigDecimal SociaLevyBase=(BigDecimal)
		// row.getCell("SociaLevyBase").getValue();
		...
		// );
		// }else{
		// row.getCell("soLevyBaseW").getStyleAttributes().setBackground(Color.
		// white);
		// }
		// }
		// }
		// IRG征收基数（外会计算） IRG征收基数 IRGLBase 如果和系统计算不一致则标红
		// if("IRGLBaseW".equals(key)){ ywj 2018-4-24
		// BigDecimal IRGLBaseW=(BigDecimal)
		// row.getCell("IRGLBaseW").getValue();
		// BigDecimal IRGLBase=(BigDecimal) row.getCell("IRGLBase").getValue();
		// if(IRGLBaseW!=null&&IRGLBase!=null){
		// if(!(IRGLBaseW.compareTo(IRGLBase)==0)){
		// row.getCell("IRGLBaseW").getStyleAttributes().setBackground(Color.red)
		// ;
		// }else{
		// row.getCell("IRGLBaseW").getStyleAttributes().setBackground(Color.white
		// );
		// }
		// }
		// }
		// IRG扣款（外会计算） IRG扣款（个人所得税） IRGDPerson 如果和系统计算不一致则标红
		// if("IRGDeduction".equals(key)){ ywj 2018-4-24
		// BigDecimal IRGDeduction=(BigDecimal)
		// row.getCell("IRGDeduction").getValue();
		// BigDecimal IRGDPerson=(BigDecimal)
		// row.getCell("IRGDPerson").getValue();
		// if(IRGDeduction!=null&&IRGDPerson!=null){
		// if(!(IRGDeduction.compareTo(IRGDPerson)==0)){
		// row.getCell("IRGDeduction").getStyleAttributes().setBackground(Color.
		// red);
		// }else{
		// row.getCell("IRGDeduction").getStyleAttributes().setBackground(Color.
		// white);
