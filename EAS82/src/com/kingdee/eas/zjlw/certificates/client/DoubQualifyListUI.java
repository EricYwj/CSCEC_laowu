 ...
 * output class name
 */
public class DoubQualifyListUI extends AbstractDoubQualifyListUI
{
    private static final Logger logger = CoreUIObject.getLogger(DoubQualifyListUI.class);
...
//	    	filter.getFilterItems().add(new FilterItemInfo("creator.id", userInfo.getId()));
    ...
    public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
    	...
    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			...
			DoubQualifyFactory.getRemoteInstance().passAudit(new ObjectUuidPK(id), null);
		}
    ...
    public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
    	...
    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
			String id = (String) iter.next();
			DoubQualifyFactory.getRemoteInstance().unpassAudit(new ObjectUuidPK(id), null);
		}
    	EcClientHelper.showOprtOK(this);
    	showOKMsgAndRefresh();
    	checkDqDate();
     ...
     */
    public DoubQualifyListUI() throws Exception
    {
        super();
    }
    ...
    //设置界面日期预警颜色
    private void checkDqDate() throws ParseException {
//    	Date date =new Date();
...
//    		//1.公证开始办理时间颜色预警；条件：分配工种完成时间不为空且不是停办
//    		if(row.getCell("entrys.assignDate").getValue()!=null&&row.getCell("entrys.ifstop").getValue().equals(false)){
//    			if(row.getCell("entrys.notiDate").getValue()==null){
//    				//值为空时：1.预警黄色:5天<=today-分配工种完成时间<=7天；2.预警红色:today-分配工种完成时间>7天。
...
//    				if(dayCou>=5 && dayCou<=7){
//    					row.getCell("entrys.notiDate").getStyleAttributes().setBackground(Color.yellow);
//    				}else if(dayCou>7){
//    					row.getCell("entrys.notiDate").getStyleAttributes().setBackground(Color.red);
//    				}
//    			}else{
//    				//不为空时：1.预警红色：公证开始办理时间-分配工种完成时间>7天。此预警一直保留。
//    				Date notiDate=(Date) row.getCell("entrys.notiDate").getValue();
//    				Date assignDate=(Date) row.getCell("entrys.assignDate").getValue();
//    				long time=(notiDate.getTime()-assignDate.getTime())/86400000;
//    				int dayCou = Integer.parseInt(String.valueOf(time)); 
//    				if(dayCou>7){
//    					row.getCell("entrys.notiDate").getStyleAttributes().setBackground(Color.red);
//    				}
//    			}
//    		}
//    		//2.公证办理完毕时间颜色预警；条件：公证开始办理时间不为空且不是停办
//    		if(row.getCell("entrys.notiDate").getValue()!=null&&row.getCell("entrys.ifstop").getValue().equals(false)){
//    			if(row.getCell("entrys.notaDate").getValue()==null){
//    				//值为空时：1.黄色预警：7天<=today-公证开始办理时间<=13天；2.红色预警:today-公证书开始办理时间>13天.
//    				Date notiDate=(Date) row.getCell("entrys.notiDate").getValue();
//    				long time=(date.getTime()-notiDate.getTime())/86400000;
//    				int dayCou = Integer.parseInt(String.valueOf(time)); 
//    				if(dayCou>=7 && dayCou<=13){
//    					row.getCell("entrys.notaDate").getStyleAttributes().setBackground(Color.yellow);
//    				}else if(dayCou>13){
//    					row.getCell("entrys.notaDate").getStyleAttributes().setBackground(Color.red);
//    				}
//    			}else{
//    				//值不为空时：1.红色预警：公证书办理完毕时间-公证书开始办理时间>13天。
//    				Date notaDate=(Date) row.getCell("entrys.notaDate").getValue();
//    				Date notiDate=(Date) row.getCell("entrys.notiDate").getValue();
//    				long time=(notaDate.getTime()-notiDate.getTime())/86400000;
//    				int dayCou = Integer.parseInt(String.valueOf(time));
//    				if(dayCou>13){
//    					row.getCell("entrys.notaDate").getStyleAttributes().setBackground(Color.red);
//    				}
...
//    			}
//    		}
//    		//3.认证办理完毕时间颜色预警；条件：公证办理完毕时间不为空且不是停办
//    		if(row.getCell("entrys.notaDate").getValue()!=null&&row.getCell("entrys.ifstop").getValue().equals(false)){
//    			if(row.getCell("entrys.qualDate").getValue()==null){
//    				//值为空时：1.黄色预警：50天<=today-公证书办理完毕时间<=65天；2.红色预警:today-公证书办理完毕时间>65天.
//    				Date notaDate=(Date) row.getCell("entrys.notaDate").getValue();
//    				long time=(date.getTime()-notaDate.getTime())/86400000;
//    				int dayCou = Integer.parseInt(String.valueOf(time)); 
//    				if(dayCou>=50 && dayCou<=65){
//    					row.getCell("entrys.qualDate").getStyleAttributes().setBackground(Color.yellow);
//    				}else if(dayCou>65){
//    					row.getCell("entrys.qualDate").getStyleAttributes().setBackground(Color.red);
//    				}
//    			}else{
//    				//值不为空时：1.红色预警：认证办理完毕时间-公证书办理完毕时间>65天。
//    				Date qualDate=(Date) row.getCell("entrys.qualDate").getValue();
//    				Date notaDate=(Date) row.getCell("entrys.notaDate").getValue();
//    				long time=(qualDate.getTime()-notaDate.getTime())/86400000;
//    				int dayCou = Integer.parseInt(String.valueOf(time));
//    				if(dayCou>65){
//    					row.getCell("entrys.qualDate").getStyleAttributes().setBackground(Color.red);
//    				}
...
//     	}
    ...
    /*added by czq on 20170710 start*/
...
//		
	...
	protected FilterInfo getOrgTreeQuery() {
		...
		if (orgUnitInfo != null) {
			...
			if ("F".equals(orgFrsNum)) {
				filterMark = "entrys.pmtProj.longNumber"; //指标项目
			}else{
    ...
    public void actionEdit_actionPerformed(ActionEvent e) throws Exception
    {
    	...
    	//获取列表界面选取的集合，同时校验是否存在补办和正常办理同时编辑的情况
		List pkList = EcClientHelper.getSelectedIdValues(tblMain,getKeyFieldName());
		if (pkList.size() > 1) {
			//循环集合，取出分录集合
	    	...
	    	for (Iterator iter = pkList.iterator(); iter.hasNext();) {
				String id = (String) iter.next();
				DoubQualifyInfo info = DoubQualifyFactory.getRemoteInstance().getDoubQualifyInfo(new ObjectUuidPK(id));
				if (info.isIfNeed()) {//存在补办的情况
					unoMark++;
				}else{//正常情况
					normalMark++;
				}
				if (normalMark != 0 && unoMark != 0 && unoMark == normalMark) {
					MsgBox.showInfo("补办人员不允许和正常办理人员一起编辑，请重新选择！");
					abort();
				}
				DoubQualifyEntryCollection antiEtyCol = info.getEntrys();
				for (int i = 0; i < antiEtyCol.size(); i++) {
					DoubQualifyEntryInfo etyInfo = antiEtyCol.get(i);
					etys.add(etyInfo);
				}
			}
	    	//进行参数传递
	        ...
	        uiContext.put("etys", etys);
	       	IUIWindow ui = UIFactory.createUIFactory(UIFactoryName.NEWTAB).create("com.kingdee.eas.zjlw.certificates.client.DoubEcEditUI", uiContext, null, OprtState.ADDNEW);
	       	ui.show();
		}else{
