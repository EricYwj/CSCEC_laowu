...
import com.kingdee.eas.zjlw.baseinfo.SecuProfFactory;
import com.kingdee.eas.zjlw.baseinfo.SecuProfInfo;

/**
 * output class name
 */
public class ProjSecuProfListUI extends AbstractProjSecuProfListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ProjSecuProfListUI.class);
...
//    	refreshList();
		...
		if ("L2".equals(strLanguage) || "l2".equals(strLanguage)) {
			...
			setUITitle("Liste des fonctions SS(projet)");
		}
		// 修改title ywj 2017-11-28===========================================END
    }
//    private void checkPrice() throws BOSException {
//    	int rowCount=tblMain.getRowCount();
//    	for(int i=0;i<rowCount;i++){
//	    	IRow row=tblMain.getRow(i);
//	    	String secuProf=(String) row.getCell("profType.name").getValue();
//	    	FilterInfo filter=new FilterInfo();
//	    	EntityViewInfo view=new EntityViewInfo();
//	        view.setFilter(filter);
//	    	
//	    	filter.getFilterItems().add(new FilterItemInfo("name",secuProf));
//	    	SecuProfInfo spInfo=SecuProfFactory.getRemoteInstance().getSecuProfCollection(view).get(0);
//	    	if(spInfo!=null){
//	    		row.getCell("nBasePay").setValue(spInfo.getBasePay());
//	    	}
//      	}
//    	
//	}
    
    //项目社保工种库基本工资和标准工资不一致的标红checkColor方法  rui_xu
    protected void checkColor() {
    	int rowCount = tblMain.getRowCount();
    	for(int i=0;i<rowCount;i++){
    		IRow row = tblMain.getRow(i);
    		if(row!=null && row.getCell("basePay")!=null && row.getCell("basePay").getValue()!=null &&
    				row.getCell("nBasePay")!=null && row.getCell("nBasePay").getValue()!=null){
    			BigDecimal basePay = (BigDecimal) row.getCell("basePay").getValue();
    			BigDecimal nBasePay = (BigDecimal) row.getCell("nBasePay").getValue();
    			if(basePay.compareTo(nBasePay)!=0){
    				row.getCell("basePay").getStyleAttributes().setBackground(java.awt.Color.red);
    			}
    		}
    	}
    }
     ...
     */
    public ProjSecuProfListUI() throws Exception
    {
        super();
    }
