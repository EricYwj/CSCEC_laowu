 ...
 * output class name
 */
public class PmtAppListUI extends AbstractPmtAppListUI {
	private static final Logger logger = CoreUIObject
			.getLogger(PmtAppListUI.class);
	 ...
	 */
	public PmtAppListUI() throws Exception {
		super();
	}
	...
	public void checkColor() throws ParseException {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			Date pmtDelrDate = (Date) row.getCell("pmtDelrDate").getValue();// 用工指标递交申请时间
			Date pmtConfirmDate = (Date) row.getCell("pmtConfirmDate").getValue();// 用工指标批复时间

			// 1.用工指标递交申请时间值为零时：
			if (pmtDelrDate == null) {
				if (actBeginDate != null) {
					long time = (newDate.getTime() - actBeginDate.getTime()) / 86400000;
					// 黄色预警：5天<=today-开工令时间<=20天；
					if (time >= 5 && time <= 20) {
						row.getCell("pmtDelrDate").getStyleAttributes().setBackground(Color.yellow);
					}
					// 红色预警：today-开工令时间>20天。
					if (time > 20) {
						row.getCell("pmtDelrDate").getStyleAttributes().setBackground(Color.red);
					}
				}
			}
			// 2.用工指标批复时间值不为零时：
			if (pmtDelrDate != null) {
				// 红色预警：用工指标递交申请时间-开工令时间>20天。
				if (actBeginDate != null) {
					long time = (pmtDelrDate.getTime() - actBeginDate.getTime()) / 86400000;
					if (time > 20) {
						row.getCell("pmtDelrDate").getStyleAttributes().setBackground(Color.red);
					}
				}
			...
			// 1.用工指标批复时间值为零时：
			if (pmtConfirmDate == null) {
				if (actBeginDate != null) {
					long time = (newDate.getTime() - actBeginDate.getTime()) / 86400000;
					// 黄色预警：60天<=today-开工令时间<=80天；
					if (time >= 60 && time <= 80) {
						row.getCell("pmtConfirmDate").getStyleAttributes().setBackground(Color.yellow);
					}
					// 红色预警：today-开工令时间>80天。
					if (time > 80) {
						row.getCell("pmtConfirmDate").getStyleAttributes().setBackground(Color.red);
					}
				}
			}
			// 2.用工指标批复时间值不为零时：
			if (pmtConfirmDate != null) {
				// 红色预警：用工指标批复时间-开工令时间>80天。
				if (actBeginDate != null) {
					long time = (pmtConfirmDate.getTime() - actBeginDate.getTime()) / 86400000;
					if (time > 80) {
						row.getCell("pmtConfirmDate").getStyleAttributes().setBackground(Color.red);
					}
				}
