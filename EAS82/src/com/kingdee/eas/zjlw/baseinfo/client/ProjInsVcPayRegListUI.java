 ...
 * output class name
 */
public class ProjInsVcPayRegListUI extends AbstractProjInsVcPayRegListUI
{
    private static final Logger logger = CoreUIObject.getLogger(ProjInsVcPayRegListUI.class);
    private TreeNode defaultTreeNode = null;
    
    /**
     * output class constructor
     */
    public ProjInsVcPayRegListUI() throws Exception
    {
        super();
    }
    ...
    public void checkColor() throws ParseException {
    	...
    	for(int i=0;i<rowCount;i++){
    		...
    		Date actBeginDate=(Date) row.getCell("actBeginDate").getValue();//开工令时间
    		Date InsRegAplDate=(Date) row.getCell("InsRegAplDate").getValue();//项目社保注册申请时间
    		Date vcPayRegAplDate=(Date) row.getCell("vcPayRegAplDate").getValue();//项目休假工资注册申请时间
    		//1.项目社保注册申请时间值为零时：
    		if(InsRegAplDate==null){
    			//黄色预警：5天<=today-开工令时间<=10天；
    			if(actBeginDate!=null){
    				long time=(newDate.getTime()-actBeginDate.getTime())/86400000;
    				if(time>=5&&time<=10){
    					row.getCell("InsRegAplDate").getStyleAttributes().setBackground(Color.yellow);
    				}
    				//红色预警：today-开工令时间>10天。
    				if(time>10){
    					row.getCell("InsRegAplDate").getStyleAttributes().setBackground(Color.red);
    				}
    			}
    		...
    		//1.项目社保注册申请时间值不为零时：
    		if(InsRegAplDate!=null){
    			//红色预警：项目社保注册申请时间-开工令时间>10天。
    			if(actBeginDate!=null){
    				long time=(InsRegAplDate.getTime()-actBeginDate.getTime())/86400000;
    				if(time>10){
    					row.getCell("InsRegAplDate").getStyleAttributes().setBackground(Color.red);
    				}
    			}
    		}
    		//2.项目休假工资注册申请时间值为零时：黄色预警：5天<=today-开工令时间<=10天；红色预警：today-开工令时间>10天。
    		if(vcPayRegAplDate==null){
    			if(actBeginDate!=null){
    				long time=(newDate.getTime()-actBeginDate.getTime())/86400000;;
    				if(time>=5 &&time<=10){
    					row.getCell("vcPayRegAplDate").getStyleAttributes().setBackground(Color.yellow);
    				}else if(time>10){
    					row.getCell("vcPayRegAplDate").getStyleAttributes().setBackground(Color.red);
    				}
    			}
    		}
	...
	protected FilterInfo getOrgTreeQuery() {
		...
		if (orgUnitInfo != null) {
			...
			projectOrgFilter.getFilterItems().add(
					new FilterItemInfo("projName.longNumber", orgUnitInfo
							.getLongNumber()
							+ "!%", CompareType.LIKE));
