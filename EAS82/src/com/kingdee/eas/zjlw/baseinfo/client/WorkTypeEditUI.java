 ...
 * output class name
 */
public class WorkTypeEditUI extends AbstractWorkTypeEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(WorkTypeEditUI.class);
    @Override
    ...
    @Override
    protected void chkisForbidden_itemStateChanged(ItemEvent e)
    		throws Exception {
    	WorkTypeInfo info = this.editData;
    	if(chkisForbidden.isSelected()){//禁用
    	...
    	super.chkisForbidden_itemStateChanged(e);
    }
    /**
     * output class constructor
     */
    public WorkTypeEditUI() throws Exception
    {
        super();
    }
    /**
    ...
    {
        WorkTypeInfo info = this.editData;
        WorkTypeFactory.getRemoteInstance().cancel(new ObjectUuidPK(info.getId()), null);
    }
    ...
    {
    	WorkTypeInfo info = this.editData;
        WorkTypeFactory.getRemoteInstance().cancelCancel(new ObjectUuidPK(info.getId()), null);
    }
    ...
    {	
    	...
    	filter.getFilterItems().add(new FilterItemInfo("type", this.editData.getId().toString()));
    	view.setFilter(filter);
    	ProjectWorkCollection col = ProjectWorkFactory.getRemoteInstance().getProjectWorkCollection(view);
    	if(col!=null && col.size()>0){
    		MsgBox.showInfo("该工种已被项目工种库应用，不允许删除！");
    		this.abort();
    	}
