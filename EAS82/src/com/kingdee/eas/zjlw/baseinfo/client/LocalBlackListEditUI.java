 ...
 * output class name
 */
public class LocalBlackListEditUI extends AbstractLocalBlackListEditUI
{
    private static final Logger logger = CoreUIObject.getLogger(LocalBlackListEditUI.class);
    
    /**
     * output class constructor
     */
    public LocalBlackListEditUI() throws Exception
    {
        super();
    }
    /**
    ...
    private void checkNumbers() throws BOSException {
    	...
    	if(txtsocialNo.getText()==null||"".equals(txtsocialNo.getText())){
    		MsgBox.showInfo("社保号不允许为空！");
    		this.abort();
    	}
    	if(pkjoinTime.getValue()==null){
    		MsgBox.showInfo("加入日期不允许为空！");
    		this.abort();
    	}
    	if(txtreason.getText()==null||"".equals(txtreason.getText())){
    		MsgBox.showInfo("加入不合格人员名单事由不允许为空！");
    		this.abort();
    	}
    	filter.getFilterItems().add(new FilterItemInfo("socialNo", txtsocialNo.getText()));
    	...
    	if(col!=null && col.size()>0){
    		MsgBox.showInfo("已存在该社保号相同的人员，不允许重复添加！");
    		this.abort();
    	}
    ...
    protected com.kingdee.bos.dao.IObjectValue createNewData()
    {
    	...
    	view.setFilter(filter);
    	CountryCollection countryCol;
		try {
			countryCol = CountryFactory.getRemoteInstance().getCountryCollection(view);
			if(countryCol!=null && countryCol.size()>0){
				objectValue.setCountry(countryCol.get(0));
			}
