 ...
 * output class name
 */
public class InsurePersonListUI extends AbstractInsurePersonListUI {
	private static final Logger logger = CoreUIObject.getLogger(InsurePersonListUI.class);
	...
	public void actionUpdate_actionPerformed(ActionEvent e) throws Exception {
		InsurePersonFactory.getRemoteInstance().updateErrData();
	}
	...
	protected FilterInfo getDefaultFilterForQuery() {
		...
		if (userOrgIdList.size() != 0) {
			filter.getFilterItems().add(new FilterItemInfo("permitOrg.id", userOrgIdSet, CompareType.INCLUDE));
	...
	public void onLoad() throws Exception {
		...
		if ("L2".equals(strLanguage) || "l2".equals(strLanguage)) {
			...
			setUITitle("La liste des employés assurés (Algérien)");
		}
	...
	public void checkColor() {
		...
		// Date secuRegDate=(Date)
		// row.getCell("entrys.secuRegDate").getValue();//社保注册时间
		// Date approachTime=(Date)
		// row.getCell("entrys.approachTime").getValue();//进场时间
		// Date date =new Date();
		// //社保注册时间
		// //值为零时
		...
		// if(time0>=5&&time0<=10){
		// row.getCell("entrys.secuRegDate").getStyleAttributes().setBackground(
		// Color.yellow);
		// }
		// //红色预警：today-进场时间>10天.
		// if(time0>10){
	 ...
	 */
	public InsurePersonListUI() throws Exception {
		super();
	}
	...
	public void actionAddNew_actionPerformed(ActionEvent e) throws Exception {
		if (this.tblMain.getRowCount() != 0) {
			MsgBox.showInfo("所选项目已存在参保人员，不允许重复新增！");
			abort();
		}
